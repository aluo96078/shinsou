...
struct MigrationSearchScreen: View {
    ...
    var body: some View {
        ScrollView {
            if viewModel.isSearching && viewModel.results.isEmpty {
                ...
                VStack(spacing: 12) {
                    ...
                    Text(MR.strings.migrationNoResults)
                        .foregroundStyle(.secondary)
                }
                .frame(maxWidth: .infinity)
                .padding(.top, 60)
        ...
        .navigationTitle(MR.strings.migrationMigrateTitle(manga.title))
        ...
        .alert(MR.strings.migrationComplete, isPresented: $viewModel.migrationSuccess) {
            Button(MR.strings.commonOk) { dismiss() }
        } message: {
            Text(MR.strings.migrationCompleteMsg)
        }
        .alert(MR.strings.migrationFailed, isPresented: .init(
            get: { viewModel.migrationError != nil },
            set: { if !$0 { viewModel.migrationError = nil } }
        )) {
            Button(MR.strings.commonOk) { viewModel.migrationError = nil }
        } message: {
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                if group.isLoading {
                    ...
                    Text(MR.strings.migrationResultsCount(group.results.count))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private var migrationOverlay: some View {
        ZStack {
            ...
            VStack(spacing: 16) {
                ProgressView()
                    .tint(.white)
                Text(MR.strings.migrationMigrating)
