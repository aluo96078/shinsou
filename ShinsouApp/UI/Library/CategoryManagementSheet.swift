...
struct CategoryManagementSheet: View {
    ...
    @State private var editingCategory: ShinsouDomain.Category? = nil
    ...
    private func moveCategories(from source: IndexSet, to destination: Int) {
        ...
        let reindexed = categories.enumerated().map { offset, cat in
            ShinsouDomain.Category(id: cat.id, name: cat.name, sort: offset, flags: cat.flags)
        }
        categories = reindexed
        persistOrder(reindexed)
    }

    private func persistOrder(_ ordered: [ShinsouDomain.Category]) {
        Task {
            for cat in ordered {
                do {
