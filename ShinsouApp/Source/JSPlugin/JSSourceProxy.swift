    ...
    private func callMangasPageFunction(_ name: String, page: Int, query: String? = nil, filters: FilterList? = nil) async throws -> MangasPage {
        return try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global().async { [weak self] in
                ...
                print("[JSSourceProxy] Calling \(name) with page=\(page), argCount=\(args.count)")
                ...
                print("[JSSourceProxy] \(name)(page=\(page)) returned \(mangas.count) mangas, hasNext=\(hasNextPage)")
