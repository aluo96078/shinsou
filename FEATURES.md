...
- **Bridge API**：`JSBridge` 提供原生能力給 JS 插件
  - HTTP 請求（GET/POST，含自訂 Header，自動攜帶 per-source cookies）
  - DOM 解析（基於 SwiftSoup 的 handle-based API）
  - Cookie 管理（getCookie / getCookies / setCookie / deleteCookie / clearCookies）
  - 帳號密碼（getCredentialUsername / getCredentialPassword / setCredential / clearCredential / hasCredential）
  - 偏好設定讀寫（getPreference / setPreference）
  - 日誌記錄
- **登入支援**：插件可宣告 `supportsLogin: true`，App 設定畫面顯示登入 UI
  - 登入呼叫插件 `source.login(username, password)` 方法
  - 登出呼叫插件 `source.logout()` 方法
  - HTTP 回應的 `Set-Cookie` 自動儲存至 per-source cookie jar
- **Per-source Cookie 隔離**：每個來源獨立的 cookie jar
  - 自動持久化至 UserDefaults，App 重啟後仍有效
  - HTTP 請求自動攜帶對應來源的 cookies
  - 支援手動新增、匯入瀏覽器 Cookie 檔案（Netscape cookies.txt / JSON）
...
#### 5.2.6 Per-source 來源設定
- 所有來源長按即可進入「來源設定」畫面（`SourceSettingsScreen`）
- **網路覆蓋**：DoH / Cloudflare Workers Proxy（3 態：跟隨全域 / 強制開 / 強制關）
- **帳號密碼**：儲存帳密供插件使用；支援登入的插件會呼叫 `login()` 方法
- **Cookie 管理**：檢視已存 cookies、手動新增、匯入瀏覽器 Cookie 檔案、刪除單筆或全部清除
- **偏好設定**：`ConfigurableSource` 定義的 toggle / text / select / multi-select 控制項
...
### 13.9 網路與進階設定
- DNS over HTTPS（DoH）— 全域開關，透過 Cloudflare 1.1.1.1 JSON DNS API 解析
- Cloudflare Workers Proxy — 全域開關，透過自訂 Worker URL 轉發請求
- Per-source 網路覆蓋 — 每個來源可獨立切換 DoH / Proxy（跟隨全域 / 強制開 / 強制關）
- Per-host 速率限制 — Token bucket 演算法，防止過度請求
- User-Agent 輪換 — Per-host 黏著式隨機 UA
- 自訂 User-Agent
- 資料庫清理功能
