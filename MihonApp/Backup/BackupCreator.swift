...
/// 負責將目前書庫狀態序列化為 `.mihonbackup` 壓縮備份檔案。
...
final class BackupCreator {
    ...
    func createBackup() async throws -> URL {
        ...
        let backup = MihonBackup(
            version: MihonBackup.currentVersion,
    ...
    private func buildBackupManga(
        manga: Manga,
        userCategories: [MihonDomain.Category],
        sortToId: [Int: Int64]
    ...
    /// 組裝備份檔案的目標路徑（Documents/Backups/mihon_YYYYMMDD_HHmmss.mihonbackup）
    private func buildBackupFileURL() -> URL {
        ...
        let filename = "mihon_\(timestamp).mihonbackup"
