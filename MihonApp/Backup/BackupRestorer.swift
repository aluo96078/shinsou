...
final class BackupRestorer {
    ...
    /// - Parameter url: `.mihonbackup` 檔案路徑
    ...
    /// - Parameter url: `.mihonbackup` 檔案的路徑（支援本機與 Files 共享）
    /// - Returns: 還原結果摘要
    ...
    /// - Parameters:
    ///   - url: `.mihonbackup` 檔案的路徑（支援本機與 Files 共享）
    ...
    /// 讀取、解壓縮並解碼備份檔案。
    private func loadBackup(from url: URL) throws -> MihonBackup {
        // 讀取檔案資料
        let rawData: Data
        do {
        ...
        do {
            backup = try JSONDecoder().decode(MihonBackup.self, from: jsonData)
        } catch {
            throw BackupRestorerError.decodingFailed(error)
        }

        // 版本檢查
        guard backup.version <= MihonBackup.currentVersion else {
    ...
    private func restoreCategories(_ backupCategories: [BackupCategory]) async throws -> [Int: Int64] {
        ...
        let existingByName: [String: MihonDomain.Category] = Dictionary(
