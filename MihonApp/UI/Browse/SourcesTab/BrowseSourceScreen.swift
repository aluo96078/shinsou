                ...
                VStack(spacing: 12) {
                    ...
                    if viewModel.isCloudflareBlocked {
                        Button {
                            ...
                            Label("開啟網頁驗證", systemImage: "globe")
...
final class BrowseSourceViewModel: ObservableObject {
    ...
    @Published var selectedTab = 0 { didSet { if oldValue != selectedTab { Task { await loadInitial() } } } }
    ...
    func loadInitial() async {
        ...
        do {
            ...
            if selectedTab == 0 {
                result = try await source.getPopularManga(page: 1)
            } else {
                result = try await source.getLatestUpdates(page: 1)
            }
    ...
    func searchWithFilters() async {
        ...
        do {
            let result = try await source.getSearchManga(page: 1, query: searchQuery, filters: filterState.filters)
            mangas = result.mangas
            hasNextPage = result.hasNextPage
        } catch {
            errorMessage = error.localizedDescription
