...
// Explicitly disambiguate LibraryItem from MihonDomain (avoids conflict when
// both MihonDomain and MihonUI are linked to the same target).
typealias LibraryItem = MihonDomain.LibraryItem

@MainActor
final class LibraryViewModel: ObservableObject {
    // MARK: - Published State
    @Published var libraryItems: [Int64: [LibraryItem]] = [:] // categoryId -> items
    @Published var categories: [MihonDomain.Category] = []
    ...
    private func startObserving() {
        ...
        categoryObservationTask = Task { [weak self] in
            ...
            for await cats in stream {
                ...
                let allCategories: [MihonDomain.Category] = [MihonDomain.Category(id: 0, name: "Default", sort: 0, flags: 0)]
