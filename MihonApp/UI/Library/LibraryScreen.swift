...
struct LibraryScreen: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .searchable(text: $viewModel.searchQuery, prompt: "Search library")
    ...
    @ViewBuilder
    private func libraryPage(for category: MihonDomain.Category) -> some View {
    ...
    private var toolbarContent: some ToolbarContent {
        if viewModel.isSelectionMode {
            ToolbarItem(placement: .topBarLeading) {
                Text("\(viewModel.selectedMangaIds.count) selected")
                    .font(.headline)
            }
