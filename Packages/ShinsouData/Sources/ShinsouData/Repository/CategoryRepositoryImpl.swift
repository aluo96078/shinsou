...
public final class CategoryRepositoryImpl: CategoryRepository {
    ...
    public func getAll() async throws -> [ShinsouDomain.Category] {
    ...
    public func getCategoriesForManga(mangaId: Int64) async throws -> [ShinsouDomain.Category] {
        try await dbPool.read { db in
            let sql = """
                SELECT category.*
                FROM category
    ...
    public func observeAll() -> AsyncStream<[ShinsouDomain.Category]> {
